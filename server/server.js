...
function status(id){const r=rooms.get(id);if(!r)return;const peers=[...r.peers];const cs=peers.filter(p=>p.role==='controller'),rs=peers.filter(p=>p.role==='receiver'),displays=peers.filter(p=>p.role==='display'),androids=peers.filter(p=>p.role==='android');const players=cs.map(p=>({player:p.player,profile:p.profile||null,name:p.name||`Player ${p.player}`}));for(const p of peers)p.send({type:'status',controllers:cs.length,receivers:rs.length,displays:displays.length,androids:androids.length,players});}
function targetsFor(ws,room){if(ws.role==='android')return [...room.peers].filter(p=>p.role==='display');if(ws.role==='display')return [...room.peers].filter(p=>p.role==='android');if(ws.role==='controller')return [...room.peers].filter(p=>p.role==='receiver');if(ws.role==='receiver')return [...room.peers].filter(p=>p.role==='controller');return [];}
function handleMessage(ws,msg){
  if(msg.type==='join'){
    release(ws);const id=cleanRoom(msg.room);if(id.length!==6)return ws.send({type:'error',message:'Invalid room code'});const room=getRoom(id);const allowed=new Set(['controller','receiver','display','android']);ws.room=id;ws.role=allowed.has(msg.role)?msg.role:'controller';ws.name=String(msg.name||'').slice(0,64);room.peers.add(ws);
    if(ws.role==='controller'){ws.player=assignPlayer(room,ws,msg.player);if(!ws.player){room.peers.delete(ws);return ws.send({type:'error',message:'This room already has four controllers.'});}ws.send({type:'joined',room:id,player:ws.player,role:ws.role});}else ws.send({type:'joined',room:id,role:ws.role});status(id);return;
  }
  if(!ws.room||!rooms.has(ws.room))return;
  const room=rooms.get(ws.room);
  if(msg.type==='profile'&&ws.role==='controller')ws.profile=String(msg.profile||'').slice(0,24);
  const packet={...msg,player:ws.player||msg.player||0,profile:msg.profile||ws.profile||null};
  for(const peer of targetsFor(ws,room))peer.send(packet);
  if(msg.type==='profile')status(ws.room);
}
...
const server=http.createServer(async(req,res)=>{
  ...
  if(u.pathname==='/api/info'){res.writeHead(200,{'Content-Type':'application/json','Cache-Control':'no-store'});return res.end(JSON.stringify({name:'RetroPad',version:VERSION,port:PORT,ips:localIPs(),maxPlayers:4,qr:true,webrtc:true}));}
  if(u.pathname==='/api/qr'){
    ...
    try{const svg=await QRCode.toString(text,{type:'svg',margin:4,errorCorrectionLevel:'M'});res.writeHead(200,{'Content-Type':'image/svg+xml; charset=utf-8','Cache-Control':'no-store'});return res.end(svg);}catch(e){res.writeHead(500,{'Content-Type':'text/plain'});return res.end('QR error');}
  }
  let requestPath=decodeURIComponent(u.pathname);if(requestPath==='/')requestPath='/display.html';const filePath=path.normalize(path.join(ROOT,requestPath));if(!filePath.startsWith(ROOT)){res.writeHead(403);return res.end('Forbidden');}fs.readFile(filePath,(err,data)=>{if(err){res.writeHead(404);return res.end('Not found');}res.writeHead(200,{'Content-Type':MIME[path.extname(filePath)]||'application/octet-stream','Cache-Control':'no-store'});res.end(data);});
});
...
server.listen(PORT,'0.0.0.0',()=>{console.log(`RetroPad v${VERSION} running on port ${PORT}`);for(const ip of localIPs())console.log(`Display:  http://${ip}:${PORT}/display.html`);console.log(`Local:    http://localhost:${PORT}/display.html`);});
